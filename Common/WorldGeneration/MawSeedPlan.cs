...
namespace apogean.Common.WorldGeneration
{
    ...
    internal sealed class MawSeedPlan
    {
        ...
        private MawSeedPlan(int seed, int[] spine, int leftSurface, int rightSurface)
        {
        ...
        internal static MawSeedPlan Create(int seed, int[] spine = null, int leftSurface = SurfaceY, int rightSurface = SurfaceY)
            => new(seed, spine, leftSurface, rightSurface);
        ...
        private void Build(int[] spine, int leftSurface, int rightSurface)
        {
            ...
            for(int group=0;group<2;group++) {
                ...
                MarkPath(new[]{a,new Point(a.X+direction*15,y+6),new Point(cx-direction*14,y+9),c},3.6,true);
            ...
            for(int x=4;x<Width-4;x++)for(int y=4;y<Height-4;y++) {
                ...
                string wall=y<Ground[x]+3?null:y<Ground[x]+8?"soil":"stone";
            ...
            int index=0;
            for(int y=Math.Max(43,Math.Max(leftSurface,rightSurface)-13);y<Height-35;y+=Next(25,34)) {
                ...
                bool BranchIntersects(int direction) {
                    ...
                    for(int x=Math.Min(bx,tx)-3;x<=Math.Max(bx,tx)+3;x++)for(int yy=cy-18;yy<=cy+10;yy++)
                ...
                Point root=new(boundary,cy),tip=new(boundary-dir*Next(15,24),cy-Next(0,16));
            ...
            for(int x=4;x<Width-4;x++)for(int y=4;y<Height-4;y++)if(bone[x,y]) {
                ...
                Cells[x,y]=new("rib",y<Ground[x]+3?null:Cells[x,y].Wall,slope,Cells[x,y].Side,true);
            }
            ...
            for(int x=mouthX-85;x<=mouthX-40;x+=2) {
                int y=Ground[x]-4;bool fits=true;
        ...
        internal bool[,] Reachable(Point? start = null)
        {
            ...
            bool Fits(int x,int y) {
                ...
                for(int dx=0;dx<2;dx++)for(int dy=0;dy<3;dy++)if(!Cells[x+dx,y+dy].Write||Cells[x+dx,y+dy].Key!=null||blocked[x+dx,y+dy])return false;
                return true;
            }
        ...
        internal void Validate()
        {
            ...
            for(int x=4;x<Width-4;x++)for(int y=4;y<Math.Min(Height-4,Ground[x]+3);y++)Check(Cells[x,y].Wall==null,"surface wall fringe");
