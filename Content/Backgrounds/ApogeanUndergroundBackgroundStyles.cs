...
namespace apogean.Content.Backgrounds
{
	...
	/// <summary>Renderer-only adapter for any biome's diagnostic underground texture contract.</summary>
	public sealed class UndergroundRenderLabBackgroundStyle : ModUndergroundBackgroundStyle
	{
		public override void FillTextureArray(int[] textureSlots)
		{
			RuinedBackgroundBiome biome =
				RuinedBackgroundSelectionSystem.Instance.UndergroundRenderLabBiome ?? RuinedBackgroundBiome.Forest;
			for (int i = 0; i < 4; i++)
			{
				textureSlots[i] = BackgroundTextureLoader.GetBackgroundSlot(Mod,
					$"Content/Backgrounds/Diagnostics/{biome}UndergroundConceptV0_{i}");
			}
		}
	}
}
