		...
		private void ConsumeLiveValidationRequest()
		{
			...
			try
			{
				...
				if (!request.StartsWith("maw-playable-", System.StringComparison.Ordinal))
					ModContent.GetInstance<MawPlayableLab>().Release();
                ...
                {
                    string variant=request.Substring("maw-rope-itemuse-".Length);
                    if(variant!="stop") Player.GetModPlayer<MawRopeItemUseProbe>().Start(variant);
                    return;
                }
				...
				if (request.StartsWith("arrival-pod-", System.StringComparison.Ordinal))
				{
					Player.GetModPlayer<WastesLandscapeCameraLab>().Release();
					ModContent.GetInstance<ForestSprayVisualLab>().Stop();
					ModContent.GetInstance<VegetationVisualLab>().Release();
