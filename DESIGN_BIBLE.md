...
The approved recovery workflow is recorded in `AUTHORING_WORKFLOW.md`. A — Snapped has a revised native component/assembly candidate under `Art/Candidates/WastesSnappedA-v2/`, awaiting review before any installation. V1 was rejected for jagged intact branches and bright bark speckles. The offline comparison is not a live screenshot or proof of wind/chop behavior.
...
- Follow-up art constraint: intact branch edges taper continuously; splintering belongs at broken ends, not randomly along the shaft. Bark must not be dotted with white/cream or glowing amber flecks. V2 proposes small dark knots and an occasional recessed hollow as wood texture, without changing collision or cutting transparent holes in the native atlas. This recess treatment still requires user approval.
