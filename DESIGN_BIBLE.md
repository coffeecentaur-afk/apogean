...
- Kessler's day-one Campus uses a compact 152x72 authored footprint inside a larger protected event reservation. Paired checkpoint towers and patrol walks mark the perimeter; their ground passages and west-side quartermaster frontage remain physically open, while a distinct internal bulkhead controls access to the armory. The two-deck headquarters, command crown, animated standards, and terrain-keyed footing must survive the same production-template render used by world generation.
