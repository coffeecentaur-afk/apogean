...
- Bounded authored chambers may contain luminous amber digestive acid. It uses animated non-solid acid tiles with exact hitbox overlap rather than an unsupported fifth liquid or a biome-wide water style. Version one damages players through a short continuous corrosion effect, does not destroy dropped items, and leaves Maw-native enemies unharmed; server-authoritative non-Maw NPC corrosion is a deferred extension. Dormancy dims and stills the acid but never makes it chemically safe. It does not flow, pump, bucket, fish, drown, or recolor unrelated water.
- Any later regional depth-pressure or breath-depletion mechanic is a separate system from digestive acid. It must have its own biome/depth predicate, equipment counters, multiplayer authority, and name rather than borrowing the acid hazard state.
