...
September7 reclamation study: `Art/Candidates/WastesReclamation-v1/README.md` holds review-only early/denser Far growth references. Both generated studies change masonry/pixel scale and have opaque backdrops; they are not installable variants. Full recovery still needs broader living vegetation than the denser study. Preserve the original ruin pixels and author registered plant layers after review. `WASTES_RECLAMATION_DIRECTION.md` now includes proposed fixed eligible-surface counting and overlapping response curves with offline arithmetic tests. These are not an approved terrain policy, art endpoint or native implementation; the installed local Forest fallback remains unchanged.
