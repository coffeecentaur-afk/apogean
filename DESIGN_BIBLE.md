...
Background corrections are durable requirements for later variants: connected silhouettes, clean material-appropriate edges, stable terrain anchoring, authored flight-visible undersides, quiet intervals between landmarks, and correct local biome/restoration selection. Preserve those requirements through the feedback-to-test workflow in AUTHORING_WORKFLOW.md and the versioned background-authoring skill. Reuse the process across families without homogenizing their artwork; every new composition still needs native game review.
