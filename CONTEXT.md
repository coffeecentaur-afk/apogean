...
A ruined-tree visual authored inside Terraria's ordinary `TileID.Trees` trunk, branch, and top atlas contracts. Height and branch placement remain native and every trunk segment remains independently choppable. The ordinary tree uses Terraria's narrow trunk/base footprint, leafless wooden tops and branches, and a deep centered crown socket that cannot separate during wind sway. A small minority may be generated by removing an upper native segment to leave a genuinely snapped trunk; oversized root flares and whole-tree overlays are separate set pieces, never part of this tree family.
