...
$records = @(Get-Content -LiteralPath $LogPath | ForEach-Object {
    if ($_ -match '((?:WASTES (?:REGIONAL GROUND|RUN MOTION (?:RESULT|SAMPLE)|HEIGHT (?:ALPHA|POSITION) (?:RESULT|SAMPLE)|CLOSE ANCHOR (?:RESULT|SAMPLE)|LAND (?:RESULT|SAMPLE)|MODULAR (?:RESULT|SAMPLE)|V1 (?:CAMERA|PROJECTION(?: SAMPLE)?|GROUND LOCK|GROUND SAMPLE|SWEEP))|FOREST (?:RESTORATION|SPRAY (?:START|RESULT))|BACKGROUND GROVE GUARD|TILE LAB (?:CAPTURE PROBE|VIEWPORT)):[^\r\n]*)$') {
        $Matches[1]
    }
})
