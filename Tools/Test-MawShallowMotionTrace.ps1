...
function Test-Trace($record) {
    if($record.schemaVersion -ne 1 -or $record.world -ne 'Apogee Native Visual V3' -or $record.player -ne 'gg') { throw 'MOTION_TRACE: context' }
    $rows=@($record.samples)
    ...
    [ordered]@{pass=$true; ticks=$rows.Count; airborneTicks=$falling; toothContactTicks=$contacts; lifeBefore=$record.startLife; lifeAfter=$record.endLife;
...
if($SelfTest -or -not $Path) {
    ...
    Write-Output 'PASS synthetic trace-validator baseline and nine rejected defects. No native game evidence.'
} else {
