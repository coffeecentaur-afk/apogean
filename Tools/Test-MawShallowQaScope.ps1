...
namespace apogean.Content.Diagnostics {
    public static class ShallowScopeChecks {
        public static int Run() {
            ...
            foreach(string request in new[]{"qa-save-and-quit","maw-shallow-pristine","maw-shallow-motion-entry","maw-shallow-light-awake","maw-shallow-light-dormant","maw-shallow-release"}) {
            ...
            for(int x=9;x<=14;x++) for(int y=19;y<=24;y++)
                Check(MawShallowQaScope.PreviewApplies(context,held,visiting,x,y,10,20,4,4)==(context&&held&&visiting&&x>=10&&x<=13&&y>=20&&y<=23),"exclusive preview bounds");
