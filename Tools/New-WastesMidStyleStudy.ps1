...
foreach($pin in @(@($source,'6CBDBE740DBD73ACF85D50672F3ED45C63887AD010A505BEB90FF73861BCB034'),
    @($station,'83C8EDE6580E6CEB2CEECD2C0F24078EFCADE3EB10680C8619BC3B1D9207C59B'),
    @($old,'B40D71FDE94AA329A77144E9114FEB4A57A9E83DA0365DBB9992DE97A365D293'))){
...
public static class MidStyleStudy {
    public static void Propose(string path,string folder,int[] selected,bool finalize){
        using(var src=new Bitmap(path)){
            int w=src.Width,h=src.Height;
            if(w!=1317||h!=1194)throw new InvalidDataException("SOURCE_DIMENSIONS");
            ...
            File.WriteAllText(Path.Combine(folder,"mask-record.json"),JsonSerializer.Serialize(new{
                sourceSHA256="6CBDBE740DBD73ACF85D50672F3ED45C63887AD010A505BEB90FF73861BCB034",
    ...
    public static void Fit(string source,string station,string old,string output){
        ...
        using(var ink=new SolidBrush(dark?Color.Gainsboro:Color.FromArgb(30,30,35))){
            ...
            string[] names={"Accepted Station (unchanged)","Previous depot (too fine-grained)","Revised depot - review candidate"};
            ...
            g.DrawString("Upper-only crops. Depot redraw fitted at 1/3, offset (32,38); approximate soil datum y340. Deep foundations NOT complete.",font,ink,16,503);
            b.Save(Path.Combine(output,dark?"Comparison-Dark.png":"Comparison-Light.png"),ImageFormat.Png);
        }
    }
}
'@
[MidStyleStudy]::Propose($source,$output,$RemoveComponents,[bool]$Finalize)
if($Finalize){
    ...
    [MidStyleStudy]::Fit((Join-Path $output 'MotorDepot-Transparent.png'),$station,$old,$output)
}
