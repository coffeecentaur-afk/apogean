...
function Check($r){
 Need ($r.schemaVersion -eq 1 -and $r.budget -in @(0,1,8,32) -and $r.player -ceq 'gg') 'Identity/budget'
 ...
 Need ($r.initialGrass -eq 4 -and $r.historyKnown -ge 0 -and $r.historyMissing -ge 0 -and $r.historyKnown+$r.historyMissing -eq 18) 'Seed/history coverage'
 ...
 for($i=0;$i -lt $r.samples.Count;$i++){
  ...
  $converted+=$s.Changed;Need ($s.Grass -eq 4+$converted -and $s.Grass -le 160) 'Cumulative grass mismatch'
  $lastUpdate=$s.GameUpdate
 }
 Need ($r.converted -eq $converted -and $r.finalGrass -eq 4+$converted) 'False endpoint'
 ...
 $expectedChanges=if($r.budget -eq 0){0}else{156}
 $pass=$null -eq $r.error -and $r.reason -ceq 'complete' -and $r.updates -eq 240 -and $r.restored -and $r.historyUnchanged -and $converted -eq $expectedChanges -and $r.finalGrass -eq 4+$expectedChanges
...
function Fixture([int]$budget){
 $remaining=if($budget -eq 0){0}else{156};$grass=4
 ...
 return @{schemaVersion=1;budget=$budget;player='gg';updates=240;samples=$rows;patch=@{X=100;Y=100;Width=32;Height=32};initialGrass=4;converted=$grass-4;finalGrass=$grass;historyKnown=9;historyMissing=9;before='A'*64;after='A'*64;historyBefore='B'*64;historyAfter='B'*64;restored=$true;historyUnchanged=$true;measuredElapsedMs=4000;error=$null;failedStep=$null;reason='complete';pass=$true;active=(Summarize-Growth @($rows|Where-Object {$_.Changed -gt 0}));idle=(Summarize-Growth @($rows|Where-Object {$_.Changed -eq 0}))}
}
