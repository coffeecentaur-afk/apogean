...
function New-Wall([string]$kind, [string]$name, [bool]$window) {
    ...
    $g = $canvas.Graphics
    for ($cellY = 0; $cellY -lt 2; $cellY++) {
        for ($cellX = 0; $cellX -lt 2; $cellX++) {
            $x = $cellX * 16; $y = $cellY * 16
            Fill $g $p[0] $x $y 16 16
            if ($window) {
                Fill $g $p[1] ($x + 1) ($y + 1) 14 14
                Fill $g $p[3] ($x + 2) ($y + 2) 12 1
                Fill $g $p[2] ($x + 2) ($y + 3) 12 10
                Fill $g $p[0] ($x + 7) ($y + 3) 2 10
                Fill $g $p[5] ($x + 3 + (($cellX + $cellY) % 3)) ($y + 5) 4 1
                Fill $g $p[0] ($x + 2) ($y + 13) 12 2
            } else {
                Fill $g $p[1] ($x + 1) ($y + 1) 14 14
                Fill $g $p[2] ($x + 2) ($y + 2) 12 12
                Fill $g $p[1] ($x + 2) ($y + 7) 12 1
                Fill $g $p[0] ($x + 7) ($y + 2) 1 13
                Fill $g $p[3] ($x + 3) ($y + 3) 3 1
                Fill $g $p[4] ($x + 10) ($y + 11) 2 2
            }
        }
    }
...
New-Wall 'Kessler' 'KesslerBulkheadWall' $false
New-Wall 'Kessler' 'KesslerWindowWall' $true
New-Wall 'Helix' 'HelixLaboratoryWall' $false
New-Wall 'Helix' 'HelixObservationWall' $true
New-Wall 'Sentrix' 'SentrixDataWall' $false
New-Wall 'Sentrix' 'SentrixWindowWall' $true
