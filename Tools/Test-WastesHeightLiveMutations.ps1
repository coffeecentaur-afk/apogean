...
$variants = @(
    ...
    @('old cap profile',$original.Replace('capFraction=0.250','capFraction=0.333').Replace('capFraction=0.500','capFraction=0.700'),$false),
...
foreach ($v in $variants) {
    ...
    if (-not $v[2] -and ($output -join "`n") -notmatch 'HEIGHT_FADE|BAD_POSITION|INCOMPLETE_FLIGHT|MISSING_RESULT|WRONG_CAP_PROFILE') { throw 'Negative trace failed for an unrelated reason' }
