...
The contract compares the control PNGs pixel-for-pixel with the known tModLoader framing references. It also rejects a custom `ModBiome` surface background paired with an unset water style, because that combination can send water-style index `-1` through Terraria's capture-camera liquid renderer.
...
The next safe increment is one handcrafted Wastes terrain block and one matching wall. The broad world-generation content remains frozen until that pair passes this gate.
